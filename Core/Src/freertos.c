...
void StartDefaultTask(void const *argument)
{
	...
	uint32_t width_snake = 6;
	uint32_t height_snake = 6;
	uint32_t x_cell = width_screen / width_snake;
	uint32_t y_cell = height_screen / height_snake;
	osEvent event;
	int global_flag = 0;
	oled_SetCursor(20, 10);
	oled_WriteString("GORYNICH", Font_11x18, White);
	oled_SetCursor(15, 40);
	oled_WriteString("Goyda to start", Font_7x10, White);
	oled_UpdateScreen();
	osDelay(100);
	...
	for (;;)
	{
		...
		for (;;)
		{
			...
			if (head->x >= x_cell || head->y >= y_cell || head->x < 0 || head->y < 0)
			{
				oled_Reset();
				oled_SetCursor(5, 10);
				oled_WriteString("Skill Issue!", Font_11x18, White);
				oled_SetCursor(40, 40);
				oled_WriteString("GIT GUT", Font_7x10, White);
				oled_SetCursor(45, 50);
				oled_UpdateScreen();
				osDelay(100);
